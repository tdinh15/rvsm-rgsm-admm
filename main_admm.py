...
parser.add_argument('--pcen', default=50, type=float, help='pruning percentage')
...
def projection(weight,percent = args.pcen,type = args.sparsity):
    if type == 'elem':
    ...
    if type == 'channel':
        ...
        # ch_weight = weight.pow(2).sum(dim=[0,2,3]).add(1e-8).pow(1/2.).detach().cpu()
        # print(weight.shape)
        # for i in range(len(ch_weight)):
            # pcen = np.nanpercentile(abs(ch_weight),percent)
...
if __name__ == '__main__':
    ...
    for epoch in range(nepoch):
        ...
        if acc > best_acc:
            ...
            torch.save(state, './weights/admm'+str(args.num_ensembles)+str(args.model)+str(int(args.pcen))+'.pth')
            best_acc = acc
